...
namespace LavanderiaVJWeb.Controllers
{
    public class ServicioController : Controller
    {
        ...
        public ServicioController(ApplicationDbContext c)
        {
            _context = c;
        }
        ...
        public IActionResult Detalle(int boleta)
        {
            ...
            ViewBag.ServiciosDisponibles = listarServDispo().Select(serv => new SelectListItem(serv.NombreSer, serv.Id.ToString()));
            return View();
        }
        [HttpPost]
        public IActionResult Detalle(Servicio detServ, int idboleta)
        {
            ...
            {
                Debug.WriteLine("entro al if de Detalle");
                // return Json(detServ);
            ...
            else
            {
                ...
                Debug.WriteLine("entro al else de Detalle");
                if (detServ.PrecServicio <= 0)
                {
                    Debug.WriteLine("entro al if dentro del else");
        ...
        public IActionResult ModificarEstadoServicio(int nroboleta, Ticket objBoleta)
        {
            ...
            ViewBag.ESTADOS = _context.ServiciosDisponibles.ToList().Select(x => new SelectListItem(x.NombreSer, x.Id.ToString()));
        ...
        public IActionResult servicios(string filtro, string data)
        {
            ...
            {
                servicios = servicios.Where(x => x.ticket.Almacen.Nombre.Equals(data)).ToList();
            }
